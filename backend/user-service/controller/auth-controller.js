  ...
  try {
    ...
    if (accessToken) {
      try {
        ...
        const blacklisted = await redisService.blacklistToken(
          accessToken,
          ttlSeconds
        );
        if (blacklisted) {
          ...
          console.warn(
            "Failed to blacklist access token - Redis may not be available"
          );
        }
      } catch (tokenError) {
        console.error(
          "Error processing access token for blacklisting:",
          tokenError
        );
      }
    }
